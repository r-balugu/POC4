...
gblLimitForListView = 50; //Variable that holds the limit of records to be displayed in list view
...
function showObjectListForm() {
    ...
    if (frmHome.seguiObjects.selectedIndex[1] == 1) {
        getAllsymptomtype();
    }
    if (frmHome.seguiObjects.selectedIndex[1] == 2) {
        getAllsymptom();
    }
}

function eventErrorCallBack(err) {
    if (err != null && err["errorMessage"] != null) callAlert(constructErrorMsg("Error", err), "error");
    else callAlert(getMessageTemplate("generalError"), "error");
...
function setAppMenuForAppGeneration() {
    ...
    if (kony.os.deviceInfo().name == "iPhone" || kony.os.deviceInfo().name == "iPad") {
        ...
        myAppMenu[2] = ["id2", "symptom...", "dflt.png", getAllsymptomtypeFromMenu];
        myAppMenu[3] = ["id3", "symptom", "dflt.png", getAllsymptomFromMenu];
    } else {
        myAppMenu[1] = ["id1", "users", "dflt.png", getAllusersFromMenu];
        myAppMenu[2] = ["id2", "symptomtype", "dflt.png", getAllsymptomtypeFromMenu];
        myAppMenu[3] = ["id3", "symptom", "dflt.png", getAllsymptomFromMenu];
    }
...
function getMessageTemplate(operation, entityName) {
    ...
    if (operation == "syncSuccess") return ("Sync Success")
    if (operation == "syncError") return ("Sync Error")
    if (operation == "syncInitSuccess") return ("Sync Init Success")
    if (operation == "syncInitFailure") return ("Sync Init Failure")
